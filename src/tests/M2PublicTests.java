...
public class M2PublicTests {
	...
	public void testValidMoveCollectibleCellVacine() throws Exception {
		...
		try {
			...
			m[4][5] = new CollectibleCell(new Vaccine());
	...
	public void testValidMoveCollectibleCellSupply() throws Exception {
		...
		try {
			...
			m[4][5] = new CollectibleCell(new Supply());
	...
	public void testZombieAttackDirections() {
		...
		try {
			...
			testCharactersList = new ArrayList<>();
	...
	public int helperCountZombies() {
		...
		for (Cell[] element : map) {
			for (int j = 0; j < element.length; j++) {
				if (element[j] instanceof CharacterCell)
					if (((CharacterCell) element[j]).getCharacter() instanceof Zombie)
						count++;
			}
		}
	...
	public ArrayList<Object> setEnvironment() {
		try {
			...
			ArrayList<Object> medics = new ArrayList<>();
			Constructor<?> medicsConstructor = Class.forName(medicPath).getConstructor(String.class, int.class,
					int.class, int.class);
			...
			availableHeros.set(availableHeros.get(null), new ArrayList<>());
			availableHeros.set(availableHeros.get(null), medics);
			...
			ArrayList<Object> zombiesArray = new ArrayList<>();
	...
	private void resetGameStatics() throws Exception {
		...
		fd.set(fd.get(null), new ArrayList<>());
	...
	private boolean checkRandomPlaces2(String className)
		...
		for (String s1 : allStrings) {
			for (String s2 : allStrings) {
	...
	public void testStartGame10Zombies2() throws Exception {
		...
		for (Object[] element : map) {
			for (int j = 0; j < element.length; j++) {
				if (element[j] != null && element[j].getClass().equals(Class.forName(charCell))) {
					if (m2.invoke(element[j]) != null
							&& m2.invoke(element[j]).getClass().equals(Class.forName(zombiePath)))
						countZombies++;
				}
			}
		}
	...
	public void testStartGameVaccines() throws Exception {
		...
		for (Object[] element : map) {
			for (int j = 0; j < element.length; j++) {
				if (element[j] != null && element[j].getClass().equals(Class.forName(collectibleCellPath))) {
					if (m2.invoke(element[j]) != null
							&& m2.invoke(element[j]).getClass().equals(Class.forName(vaccinePath)))
						count++;
				}
			}
		}
	...
	public void testStartGameSupplyies2() throws Exception {
		...
		for (Object[] element : map) {
			for (int j = 0; j < element.length; j++) {
				if (element[j] != null && element[j].getClass().equals(Class.forName(collectibleCellPath))) {
					if (m2.invoke(element[j]) != null
							&& m2.invoke(element[j]).getClass().equals(Class.forName(supplyPath)))
						count++;
				}
			}
		}
		...
		for (Object[] element : map) {
			for (int j = 0; j < element.length; j++) {
				assertFalse("All empty cells should be initalized as empty charachter cells not null",
						element[j] == null);
				if (element[j] != null && (element[j].getClass().equals(Class.forName(collectibleCellPath)))
						|| (element[j].getClass().equals(Class.forName(trapCellPath)))) {
					count++;
				}
			}
		}
	...
	public void testEndTurnZombieAttackLeft() throws Exception {
		...
		fd = Class.forName(characterPath).getDeclaredField("currentHp");
		fd.setAccessible(true);
		assertEquals("Zombies' HP should be updated after they attack during ending the turn", 40 - dmg / 2,
				(int) fd.get(z2));
	}
	...
	public void testEndTurnZombieAttackRight2() throws Exception {
		...
		fd = Class.forName(characterPath).getDeclaredField("currentHp");
		fd.setAccessible(true);
		assertEquals("Zombies should attack any valid target to their right during ending the turn", maxHp - 10,
				(int) fd.get(h1));
	}
	...
	public void testEndTurnZombieAttackUp2() throws Exception {
		...
		fd = Class.forName(characterPath).getDeclaredField("currentHp");
		fd.setAccessible(true);
		assertEquals("Zombies should attack any valid target above them during ending the turn", maxHp - 10,
				(int) fd.get(h1));
	}
	...
	public void testEndTurnZombieAttackDown2() throws Exception {
		...
		fd = Class.forName(characterPath).getDeclaredField("currentHp");
		fd.setAccessible(true);
		assertEquals("Zombies should attack any valid target below them during ending the turn", maxHp - 10,
				(int) fd.get(h1));
	}
		...
		for (int i = 0; i < herosInRange.size(); i++) {
			...
			if (!oneAttacked && (int) fd.get(herosInRange.get(i)) < maxHpInRange.get(i)) {
				assertEquals("Zombies should attack one valid target in range during ending the turn",
						maxHpInRange.get(i) - 10, (int) fd.get(herosInRange.get(i)));
				oneAttacked = true;
			}
